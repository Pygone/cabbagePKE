...
//
int exit(int code) { return do_user_call(SYS_user_exit, code, 0, 0, 0, 0, 0, 0); }

//
// lib call to naive_malloc
//
void *naive_malloc() { return (void *)do_user_call(SYS_user_allocate_page, 0, 0, 0, 0, 0, 0, 0); }

//
// lib call to naive_free
//
void naive_free(void *va) { do_user_call(SYS_user_free_page, (uint64)va, 0, 0, 0, 0, 0, 0); }
...
// lib call to read
//
int read_u(int fd, void *buf, uint64 count) { return do_user_call(SYS_user_read, fd, (uint64)buf, count, 0, 0, 0, 0); }
...
// lib call to seek
//
int lseek_u(int fd, int offset, int whence) { return do_user_call(SYS_user_lseek, fd, offset, whence, 0, 0, 0, 0); }

//
// lib call to read file information
//
int stat_u(int fd, struct istat *istat) { return do_user_call(SYS_user_stat, fd, (uint64)istat, 0, 0, 0, 0, 0); }
...
//
int opendir_u(const char *dirname) { return do_user_call(SYS_user_opendir, (uint64)dirname, 0, 0, 0, 0, 0, 0); }

//
// lib call to read dir
//
int readdir_u(int fd, struct dir *dir) { return do_user_call(SYS_user_readdir, fd, (uint64)dir, 0, 0, 0, 0, 0); }

//
// lib call to make dir
//
int mkdir_u(const char *pathname) { return do_user_call(SYS_user_mkdir, (uint64)pathname, 0, 0, 0, 0, 0, 0); }

//
// lib call to close dir
//
int closedir_u(int fd) { return do_user_call(SYS_user_closedir, fd, 0, 0, 0, 0, 0, 0); }
...
// lib call to unlink
//
int unlink_u(const char *fn) { return do_user_call(SYS_user_unlink, (uint64)fn, 0, 0, 0, 0, 0, 0); }
...
int exec(const char *path, char *arg) { return do_user_call(SYS_user_exec, (uint64)path, (uint64)arg, 0, 0, 0, 0, 0); }

int wait(const int pid) { return do_user_call(SYS_user_wait, pid, 0, 0, 0, 0, 0, 0); }

int gets(char *buf, int len) { return do_user_call(SYS_user_gets, (uint64)buf, len, 0, 0, 0, 0, 0); }
int sem_new(int sem) { return do_user_call(SYS_user_sem_init, sem, 0, 0, 0, 0, 0, 0); }
...
void print_backtrace(int depth) { do_user_call(SYS_user_backtrace, depth, 0, 0, 0, 0, 0, 0); }

//
// lib call to read present working directory (pwd)
//
int read_cwd(char *path) { return do_user_call(SYS_user_rcwd, (uint64)path, 0, 0, 0, 0, 0, 0); }

//
// lib call to change pwd
//
int change_cwd(const char *path) { return do_user_call(SYS_user_ccwd, (uint64)path, 0, 0, 0, 0, 0, 0); }

int test(char *path) { return do_user_call(SYS_user_test, (uint64)path, 0, 0, 0, 0, 0, 0); }