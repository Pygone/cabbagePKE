...
{
    int ret;
    ret = hash_table_init(&dentry_hash_table, dentry_hash_equal, dentry_hash_func, NULL, NULL, NULL);
    if (ret != 0)
        return ret;

    ret = hash_table_init(&vinode_hash_table, vinode_hash_equal, vinode_hash_func, NULL, NULL, NULL);
    if (ret != 0)
        return ret;
    return 0;
}
...
{
    char miss_name[MAX_PATH_LEN];
    struct dentry *parent_old = parent;
    // lookup oldpath
    struct dentry *old_file_dentry = lookup_final_dentry(oldpath, &parent_old, miss_name);
    if (!old_file_dentry)
    {
    ...
    struct dentry *parent_new = parent;
    // lookup the newpath
    // note that parent is changed to be the last directory entry to be accessed
    struct dentry *new_file_dentry = lookup_final_dentry(newpath, &parent_new, miss_name);
    if (new_file_dentry)
    {
...
//
struct file *alloc_vfs_file(struct dentry *file_dentry, int readable, int writable, int offset)
{
...
int dentry_hash_equal(void *key1, void *key2)
{
    ...
    if (strcmp(dentry_key1->name, dentry_key2->name) == 0 && dentry_key1->parent == dentry_key2->parent)
    {
        return 1;
    }
    return 0;
