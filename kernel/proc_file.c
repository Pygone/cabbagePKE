...
int do_open(char* pathname, int flags)
{
	...
	if ((opened_file = vfs_open(pathname, flags, (struct dentry*) current->pfiles->cwd)) == NULL) return -1;
...
{
	struct file* opened_file = NULL;
	if ((opened_file = vfs_opendir(pathname,current->pfiles->cwd)) == NULL) return -1;
...
{
	return vfs_mkdir(pathname, (struct dentry*) current->pfiles->cwd);
}
...
{
	return vfs_link(oldpath, newpath, (struct dentry*) current->pfiles->cwd);
}
...
{
	return vfs_unlink(path, (struct dentry*) current->pfiles->cwd);
}
