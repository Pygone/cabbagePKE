...
ssize_t sys_user_exit(uint64 code)
{
    ...
    int hart_id = read_tp();
    process *proc = current[hart_id];
    free_process(proc);
    int is_child = proc->is_child;
    if (!is_child)
    {
        while (child_running)
        {
...
ssize_t sys_user_test(char *path1)
{
    int hart_id = read_tp();
    char *pfn = (char *)user_va_to_pa((pagetable_t)(current[hart_id]->pagetable), (void *)path1);
    return do_test(pfn);
}
