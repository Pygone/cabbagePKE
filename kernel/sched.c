...
void insert_to_ready_queue(process *proc)
{
    ...
    if (ready_queue_head[hart_id] == NULL)
    {
        proc->status = READY;
        proc->queue_next = NULL;
        ready_queue_head[hart_id] = proc;
        return;
    }
    ...
    for (p = ready_queue_head[hart_id]; p->queue_next != NULL; p = p->queue_next)
        if (p == proc)
...
void schedule()
{
    ...
    if (!ready_queue_head[hart_id])
    {
        ...
        for (int i = 0; i < NPROC; i++)
            if ((procs[hart_id][i].status != FREE) && (procs[hart_id][i].status != ZOMBIE))
            {
                should_shutdown = 0;
                sprint("ready queue empty, but process %d is not in free/zombie state:%d\n", i, procs[hart_id][i].status);
            }
