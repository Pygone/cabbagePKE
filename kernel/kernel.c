...
{
  // hart 0 begin with 0x8100... hart 1 begin with 0x8500...
  int hart_id = read_tp();
  // USER_TRAP_FRAME is a physical address defined in kernel/config.h

  uint64 hart_num = hart_id*0x5000000;
  proc->trapframe = (trapframe *)(USER_TRAP_FRAME+hart_num);

  memset(proc->trapframe, 0, sizeof(trapframe));
  // USER_KSTACK is also a physical address defined in kernel/config.h
  proc->kstack = USER_KSTACK+hart_num;
  proc->trapframe->regs.sp = USER_STACK+hart_num;
