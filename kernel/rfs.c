...
//
int rfs_r1block(struct rfs_device *rfs_dev, int n_block) { return dop_read(rfs_dev, n_block); }

//
// call ramdisk_write via the device structure.
// write iobuffer of rfs_dev to RAM disk at the "n_block"^th block.
//
int rfs_w1block(struct rfs_device *rfs_dev, int n_block) { return dop_write(rfs_dev, n_block); }
...
struct rfs_dinode *rfs_read_dinode(struct rfs_device *rdev, int n_inode)
{
    ...
    memcpy(dinode, (char *)rdev->iobuffer + offset * RFS_INODESIZE, sizeof(struct rfs_dinode));
    return dinode;
}
...
//
int rfs_write_dinode(struct rfs_device *rdev, const struct rfs_dinode *dinode, int n_inode)
{
    int n_block = n_inode / (RFS_BLKSIZE / RFS_INODESIZE) + RFS_BLK_OFFSET_INODE;
    int offset = n_inode % (RFS_BLKSIZE / RFS_INODESIZE);
    ...
    memcpy(rdev->iobuffer + offset * RFS_INODESIZE, dinode, sizeof(struct rfs_dinode));
...
int rfs_unlink(struct vinode *parent, struct dentry *sub_dentry, struct vinode *unlink_vinode)
{
    ...
    for (int i = delete_block_index + 1; i < parent->blocks; i++)
    {
        ...
        // of previous block
        memcpy(previous_block + one_block_direntrys - 1, rdev->iobuffer, sizeof(struct rfs_direntry));

        // move the direntry in this block forward by one
        memmove(this_block, this_block + 1, (one_block_direntrys - 1) * sizeof(struct rfs_direntry));
